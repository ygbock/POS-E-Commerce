...
import { DiscoveryServiceRequestPage } from '../discovery/DiscoveryServiceRequestPage';
...
export const Storefront: React.FC<StorefrontProps> = ({ onOpenAdmin, onOpenPos }) => {
  ...
  if (route.name === 'discover-business') {
    return (
      <div className={isDarkMode ? 'dark' : ''}>
        <DiscoveryBusinessProfile businessId={route.slug} onBack={goHome} onRequestService={(service) => { window.history.pushState({}, '', '/discover/request-service?serviceId=' + encodeURIComponent(service.id) + '&serviceName=' + encodeURIComponent(service.name)); window.dispatchEvent(new PopStateEvent('popstate')); }} />
      </div>
    );
  }
  ...
  if (route.name === 'discover-request-service') {
    ...
    return (
      <div className={isDarkMode ? 'dark' : ''}>
        <DiscoveryServiceRequestPage serviceName={params.get('serviceName') || undefined} onBack={goHome} />
      </div>
    );
  }
