...
export const ProductDiscoveryCard: React.FC<ProductDiscoveryCardProps> = ({
  ...
  return (
    <article
      className={`group rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3.5 flex flex-col justify-between shadow-xs hover:shadow-lg hover:border-slate-300 dark:hover:border-slate-700 transition-all ${className}`}
      aria-labelledby={`prod-title-${product.variant_id}`}
    >
      <div>
        {/* Product Image */}
        <div className="relative aspect-square rounded-xl bg-slate-100 dark:bg-slate-800 overflow-hidden mb-3">
          {firstImage ? (
            <img
              src={firstImage}
        ...
        <div className="space-y-1.5">
          <h3
            ...
            className="font-bold text-sm text-slate-900 dark:text-white line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors"
          >
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2">
        ...
        <a
          href={`/discover/business/${encodeURIComponent(product.business_slug || product.business_id)}`}
          onClick={(e) => {
            if (onSelect) {
              e.preventDefault();
              onSelect(product);
          ...
          className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-950/40 text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
