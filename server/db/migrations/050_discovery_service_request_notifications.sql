...
BEGIN
  ...
  IF NEW.event_type = 'QUOTE_SUBMITTED' AND request_customer IS NOT NULL THEN
    ...
    VALUES
      ('notif_' || md5(random()::text || clock_timestamp()::text || NEW.id || COALESCE(recipient.user_id, request_customer, 'customer')),NEW.request_id,NEW.id,request_customer,NEW.business_id,ntype,ntitle,nmessage,
    ...
    IF NEW.business_id IS NOT NULL THEN
      ...
      LOOP
        INSERT INTO discovery_service_request_notifications
          (id,request_id,event_id,recipient_user_id,business_id,notification_type,title,message,metadata)
        VALUES
          ('notif_' || md5(random()::text || clock_timestamp()::text || NEW.id || COALESCE(recipient.user_id, request_customer, 'customer')),NEW.request_id,NEW.id,recipient.user_id,NEW.business_id,ntype,ntitle,nmessage,
           jsonb_build_object('eventType',NEW.event_type,'quoteId',NEW.quote_id));
      END LOOP;
    END IF;
