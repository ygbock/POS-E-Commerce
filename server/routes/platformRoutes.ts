...
export function createPlatformRouter(db: DatabaseClient, injectedSubscriptionService?: SubscriptionService): Router {
  ...
  router.patch('/discovery/search-ranking', requireAuth(), requirePlatformPermission(PERMISSIONS.PLATFORM_DISCOVERY), async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!updates.length) return badRequest(res, 'NO_UPDATES', 'No ranking weight updates supplied.');
      values.push(req.auth!.userId);
      updates.push('updated_by_user_id = $' + values.length);
      updates.push('updated_at = CURRENT_TIMESTAMP');
      const r = await db.query<any>(
        `UPDATE discovery_search_ranking_config SET ${updates.join(', ')} WHERE id = 'default' RETURNING id,text_match_weight,exact_match_weight,prefix_match_weight,verified_weight,rating_weight,review_count_weight,fuzzy_match_weight,distance_penalty_weight,availability_weight,is_active,updated_by_user_id,updated_at`,
        values
      );
