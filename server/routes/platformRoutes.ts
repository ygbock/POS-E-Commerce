...
export function createPlatformRouter(db: DatabaseClient, injectedSubscriptionService?: SubscriptionService): Router {
  ...
  router.get('/discovery/search-aliases', requireAuth(), requirePlatformPermission(PERMISSIONS.PLATFORM_DISCOVERY), async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const params: any[] = [];
      const where: string[] = [];
      if (entityType) { params.push(entityType); where.push('a.entity_type =  (_req: Request, res: Response, next: NextFunction) => {
    try {
      res.json({
        success: true,
        data: {
 ...
 + params.length); }
      ...
      if (search) {
        params.push('%' + search + '%');
        where.push('(LOWER(a.alias) LIKE LOWER( (_req: Request, res: Response, next: NextFunction) => {
    try {
      res.json({
        success: true,
        data: {
 ...
 + params.length + ') OR LOWER(a.normalized_alias) LIKE LOWER( (_req: Request, res: Response, next: NextFunction) => {
    try {
      res.json({
        success: true,
        data: {
 ...
 + params.length + '))');
      ...
      const count = await db.query<{ total: number }>(
        `SELECT COUNT(*)::int AS total FROM discovery_search_aliases a ${whereSql}`, params,
      );
