...
export function createPlatformRouter(db: DatabaseClient, injectedSubscriptionService?: SubscriptionService): Router {
  ...
  router.get('/discovery/search-ranking', requireAuth(), requirePlatformPermission(PERMISSIONS.PLATFORM_DISCOVERY), async (_req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await db.query<any>('SELECT id,text_match_weight,exact_match_weight,prefix_match_weight,verified_weight,rating_weight,review_count_weight,fuzzy_match_weight,distance_penalty_weight,availability_weight,is_active,updated_by_user_id,updated_at FROM discovery_search_ranking_config WHERE id=\'default\'');
      if (!result.rows[0]) return res.status(404).json({ success: false, error: { code: 'RANKING_CONFIG_NOT_FOUND', message: 'Search ranking configuration not found.' } });
      return res.json({ success: true, data: result.rows[0] });
    } catch (err) { next(err); }
  });

  router.patch('/discovery/search-ranking', requireAuth(), requirePlatformPermission(PERMISSIONS.PLATFORM_DISCOVERY), async (req: Request, res: Response, next: NextFunction) => {
    try {
      const fields = [
        'text_match_weight','exact_match_weight','prefix_match_weight','verified_weight',
        'rating_weight','review_count_weight','fuzzy_match_weight','distance_penalty_weight','availability_weight',
      ];
      const updates: string[] = [];
      const values: any[] = [];
      const ranges: Record<string, [number, number]> = {
        text_match_weight: [0, 500],
        exact_match_weight: [0, 200],
        prefix_match_weight: [0, 200],
        verified_weight: [0, 100],
        rating_weight: [0, 20],
        review_count_weight: [0, 20],
        fuzzy_match_weight: [0, 200],
        distance_penalty_weight: [0, 10],
        availability_weight: [0, 100],
      };
      for (const field of fields) {
        ...
        if (!Number.isFinite(value) || value < range[0] || value > range[1]) {
          return badRequest(res, 'INVALID_RANKING_WEIGHT', field + ' must be a finite number between ' + range[0] + ' and ' + range[1] + '.');
        }
        values.push(value);
        updates.push(field + '= async (req: Request, res: Response, next: NextFunction) => {
    try {
      const entityType = typeof req.query.entityType === 'string' ? req.query.entityType.trim().toUpperCase() : undefined;
      if (entityType && !['BUSINESS', 'PRODUCT', 'SERVICE'].includes(entityType)) return badRequest(res, 'INVALID_ENTITY_TYPE', 'entityType must be BUSINESS, PRODUCT, or SERVICE.');
      ...
      const dollar = (n: number) => String.fromCharCode(36) + n;
      const params: any[] = [];
      const where: string[] = [];
      if (entityType) { params.push(entityType); where.push('a.entity_type = ' + dollar(params.length)); }
      if (activeOnly) where.push('a.is_active = TRUE');
      if (search) { params.push('%' + search + '%'); const p = dollar(params.length); where.push('(LOWER(a.alias) LIKE LOWER(' + p + ') OR LOWER(a.normalized_alias) LIKE LOWER(' + p + '))'); }
      const whereSql = where.length ? 'WHERE ' + where.join(' AND ') : '';
      const count = await db.query<{ total: number }>('SELECT COUNT(*)::int AS total FROM discovery_search_aliases a ' + whereSql, params);
      const pageParams = [...params, limit, offset];
      const limitParam = dollar(pageParams.length - 1);
      const offsetParam = dollar(pageParams.length);
      const rows = await db.query<any>(
        ...
        'FROM discovery_search_aliases a LEFT JOIN discovery_businesses b ON a.entity_type = \'BUSINESS\' AND b.id = a.entity_id ' +
        whereSql + ' ORDER BY a.updated_at DESC, a.id ASC LIMIT ' + limitParam + ' OFFSET ' + offsetParam,
        pageParams,
      );
  ...
  router.patch('/discovery/search-aliases/:id', requireAuth(), requirePlatformPermission(PERMISSIONS.PLATFORM_DISCOVERY), async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (req.body?.isActive !== undefined && typeof req.body.isActive !== 'boolean') return badRequest(res, 'INVALID_ACTIVE_STATE', 'isActive must be a boolean.');
      const result = await db.query<any>('UPDATE discovery_search_aliases SET is_active = COALESCE($2, is_active), updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING id, entity_type, entity_id, alias, normalized_alias, is_active, created_at, updated_at', [aliasId, req.body?.isActive === undefined ? null : req.body.isActive]);
  ...
  router.get('/discovery/search-analytics', requireAuth(), requirePlatformPermission(PERMISSIONS.PLATFORM_DISCOVERY), async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const summary = await db.query<any>(
        "SELECT COUNT(*) FILTER (WHERE event_type = 'SEARCH')::int AS searches, COUNT(*) FILTER (WHERE event_type = 'SEARCH' AND (metadata->>'zeroResults')::boolean = TRUE)::int AS zero_result_searches, COUNT(DISTINCT metadata->>'queryHash') FILTER (WHERE event_type = 'SEARCH' AND metadata->>'queryHash' IS NOT NULL)::int AS unique_queries, COUNT(*) FILTER (WHERE event_type = 'IMPRESSION')::int AS impressions, COUNT(*) FILTER (WHERE event_type IN ('VIEW','STORE_CLICK','PRODUCT_VIEW','SERVICE_VIEW'))::int AS result_engagements FROM discovery_analytics_events WHERE created_at >= CURRENT_TIMESTAMP - INTERVAL '1 day' * $1",
        [days],
      );
      const popular = await db.query<any>(
        "SELECT metadata->>'queryHash' AS query_hash, COUNT(*)::int AS searches, COUNT(*) FILTER (WHERE (metadata->>'zeroResults')::boolean = TRUE)::int AS zero_results FROM discovery_analytics_events WHERE event_type = 'SEARCH' AND created_at >= CURRENT_TIMESTAMP - INTERVAL '1 day' * $1 AND metadata->>'queryHash' IS NOT NULL GROUP BY metadata->>'queryHash' ORDER BY searches DESC, query_hash ASC LIMIT 25",
        [days],
      );
      return res.json({ success: true, data: { windowDays: days, summary: summary.rows[0] || {}, popularQueries: popular.rows, queryPrivacy: 'Only SHA-256 query hashes are returned; raw search queries are not stored.' } });
    } catch (err) { next(err); }
  });
 ...
 + values.length);
      ...
      if (req.body?.is_active !== undefined) {
        if (typeof req.body.is_active !== 'boolean') return badRequest(res, 'INVALID_RANKING_STATE', 'is_active must be boolean.');
        values.push(req.body.is_active);
        updates.push('is_active= async (req: Request, res: Response, next: NextFunction) => {
    try {
      const entityType = typeof req.query.entityType === 'string' ? req.query.entityType.trim().toUpperCase() : undefined;
      if (entityType && !['BUSINESS', 'PRODUCT', 'SERVICE'].includes(entityType)) return badRequest(res, 'INVALID_ENTITY_TYPE', 'entityType must be BUSINESS, PRODUCT, or SERVICE.');
      const activeOnly = req.query.activeOnly !== 'false';
 ...
 + values.length);
      ...
      if (!updates.length) return badRequest(res, 'NO_RANKING_CHANGES', 'At least one ranking setting is required.');
      values.push(req.auth!.userId);
      updates.push('updated_by_user_id= async (req: Request, res: Response, next: NextFunction) => {
    try {
      const entityType = typeof req.query.entityType === 'string' ? req.query.entityType.trim().toUpperCase() : undefined;
      if (entityType && !['BUSINESS', 'PRODUCT', 'SERVICE'].includes(entityType)) return badRequest(res, 'INVALID_ENTITY_TYPE', 'entityType must be BUSINESS, PRODUCT, or SERVICE.');
      const activeOnly = req.query.activeOnly !== 'false';
 ...
 + values.length);
      const result = await db.query<any>(
        'UPDATE discovery_search_ranking_config SET ' + updates.join(',') + ',updated_at=CURRENT_TIMESTAMP WHERE id=\'default\' RETURNING id,text_match_weight,exact_match_weight,prefix_match_weight,verified_weight,rating_weight,review_count_weight,fuzzy_match_weight,distance_penalty_weight,availability_weight,is_active,updated_by_user_id,updated_at',
        values,
      );
      return res.json({ success: true, data: result.rows[0] });
    } catch (err) { next(err); }
