...
export function createDiscoveryRouter(db: DatabaseClient) {
  ...
  router.get('/search', async (req,res,next)=>{
    try {
      ...
      const distanceSql = lat != null && lng != null ? `6371 * acos(LEAST(1,GREATEST(-1,cos(radians(${bind(lat)}))*cos(radians(l.latitude))*cos(radians(l.longitude)-radians(${bind(lng)}))+sin(radians(${lat}))*sin(radians(l.latitude)))))` : null;
