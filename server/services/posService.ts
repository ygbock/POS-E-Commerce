...
export class PosService {
  ...
  async checkout(params: {
    ...
    return this.db.withTransaction(async (tx) => {
      // A. Check Idempotency with exact comparison and replay protection
      if (params.idempotency_key) {
        const existingOrderRes = await tx.query<any>(
        ...
        if (existingOrderRes.rows.length > 0) {
          ...
          const fullOrder = await this.orderRepo.findOrderById(existingOrderId, organization_id, tx);
          if (fullOrder) {
            ...
            // Validate request characteristics to detect mismatch (materially different request)
            let match = true;
            
            if (
              fullOrder.order.location_id !== location_id ||
              fullOrder.order.pos_session_id !== session_id ||
              (payment && payment.payment_method !== params.payment_method)
            ) {
              match = false;
            }

            if (match) {
              if (fullOrder.items.length !== params.cart_items.length) {
                match = false;
              } else {
                const sortedReqItems = [...params.cart_items].sort((a, b) => a.variant_id.localeCompare(b.variant_id));
                const sortedDbItems = [...fullOrder.items].sort((a, b) => a.variant_id.localeCompare(b.variant_id));
                
                for (let i = 0; i < sortedReqItems.length; i++) {
                  const reqIt = sortedReqItems[i];
                  const dbIt = sortedDbItems[i];
                  
                  if (
                    reqIt.variant_id !== dbIt.variant_id ||
                    parseQtyToScaled(reqIt.quantity) !== parseQtyToScaled(dbIt.quantity.toString())
                  ) {
                    match = false;
                    break;
                  }
      ...
      for (const item of params.cart_items) {
        ...
        orderItems.push({
          ...
          sku: variant.sku,
          unit_price: Number(variant.retail_price),
          cost_price: Number(variant.cost_price),
          quantity: Number(formatScaledToQtyString(qtyScaled)),
          discount_amount: Number(formatCentsToMoneyString(lineDiscountCents)),
          tax_rate: Number(taxRateStr),
          total_amount: Number(formatCentsToMoneyString(lineTotalCents)),
        });
      }
      ...
      const order: OrderRecord & { idempotency_key?: string } = {
        ...
        subtotal: Number(formatCentsToMoneyString(totalSubtotalCents)),
        discount_amount: Number(formatCentsToMoneyString(totalDiscountCents)),
        tax_amount: Number(formatCentsToMoneyString(totalTaxCents)),
        shipping_fee: 0,
        total_amount: Number(formatCentsToMoneyString(totalAmountCents)),
        total_cost_amount: Number(formatCentsToMoneyString(totalCostCents)),
        payment_status: 'Paid',
        status: 'Completed',
        cashier_name,
      ...
      const payment: PaymentRecord = {
        ...
        amount: Number(formatCentsToMoneyString(totalAmountCents)),
        currency: 'SLE',
        status: 'Completed',
        ...
        await this.invRepo.recordMovement(
          {
            ...
            unit_cost: formatCentsToMoneyString(parseMoneyToCents(item.cost_price.toFixed(2))),
  ...
  async processReturn(params: {
    ...
    return this.db.withTransaction(async (tx) => {
      // A. Check Idempotency with replay protection
      if (params.idempotency_key) {
        const existingReturnRes = await tx.query<any>(
        ...
        if (existingReturnRes.rows.length > 0) {
          ...
          const returnRecord = {
            ...existingReturn,
            refund_amount: parseExactMoney(existingReturn.refund_amount),
          };
          const returnItems = await this.posRepo.getReturnItems(existingReturn.id, tx);

          // Validate characteristics
          let match = true;
          if (
            existingReturn.order_id !== order_id ||
            existingReturn.refund_method !== refund_method
          ) {
            match = false;
          }

          if (match) {
            if (returnItems.length !== params.return_items.length) {
              match = false;
            } else {
              const sortedReq = [...params.return_items].sort((a, b) => a.variant_id.localeCompare(b.variant_id));
              const sortedDb = [...returnItems].sort((a, b) => a.variant_id.localeCompare(b.variant_id));
              for (let i = 0; i < sortedReq.length; i++) {
                if (
                  sortedReq[i].variant_id !== sortedDb[i].variant_id ||
                  parseQtyToScaled(sortedReq[i].quantity) !== parseQtyToScaled(sortedDb[i].quantity)
                ) {
                  match = false;
                  break;
                }
        ...
        await this.invRepo.recordMovement(
          {
            ...
            unit_cost: formatCentsToMoneyString(parseMoneyToCents(origItem.cost_price.toFixed(2))),
