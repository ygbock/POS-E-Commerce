...
async function main() {
  ...
  try {
    ...
    // ------------------------------------------------------------------
    await runTest('10.1. Migration files 001-016 exist with consistent checksums', () => {
      const migrationsDir = path.join(process.cwd(), 'server', 'db', 'migrations');
      const files = fs.readdirSync(migrationsDir).filter((f) => f.endsWith('.sql')).sort();

      assert.ok(files.length >= 16, 'All migrations must exist');
      ...
      assert.strictEqual(files[14].startsWith('015_') || files[14].startsWith('016_'), true);
      assert.strictEqual(files[15].startsWith('016_'), true);
